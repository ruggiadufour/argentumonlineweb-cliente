import { generateObject } from "@/utils/genereators";
...
class Engine {
  ...
  delta: number;
  personajes: Record<User["id"], User>;
  lastDelta: number;
  ...
  initCanvas = async () => {    
    ...
    this.canvas.background.ctx = this.refCanvas.background.getContext("2d", {
      alpha: false,
    });
  ...
  clickCanvas = (pos) => {
    let tileSelected = this.inits.mapData[this.config.mapNumber][pos.y][pos.x];

    if (!tileSelected.id) {
      tileSelected =
        this.inits.mapData[this.config.mapNumber][pos.y + 1][pos.x];
    }
    ...
    if (this.config.hechizoSelected) {
      ...
      if (
        ...
        this.game.writeConsole("Aún no puedes realizar esa acción.", "gray");
      }
    }
  ...
  deletePersonaje = (idPersonaje: User["id"]) => {
    if (this.personajes[idPersonaje]) {
  ...
  hit = () => {
    ...
    switch (heading) {
      case 1:
        if (this.inits.mapData[this.config.mapNumber][y - 1][x].id !== 0) {
          valido = true;
        }
        break;
      case 2:
        ...
        break;
      case 3:
        if (this.inits.mapData[this.config.mapNumber][y][x + 1].id !== 0) {
          valido = true;
        }
        break;
      case 4:
  ...
  check = () => {
    ...
    if (
      ...
      if (
        this.keydown[
          this.ui.state.keyCodeDefault[this.config.nameKeyCode.flechaIzquierda]
        ]
      ) {
        this.moveTo(this.user.id, this.config.direcciones.left);
      } else if (
        this.keydown[
          this.ui.state.keyCodeDefault[this.config.nameKeyCode.flechaDerecha]
        ]
      ) {
        this.moveTo(this.user.id, this.config.direcciones.right);
      } else if (
        this.keydown[
          this.ui.state.keyCodeDefault[this.config.nameKeyCode.flechaArriba]
        ]
      ) {
        this.moveTo(this.user.id, this.config.direcciones.up);
      } else if (
        this.keydown[
          this.ui.state.keyCodeDefault[this.config.nameKeyCode.flechaAbajo]
        ]
      ) {
  ...
  updateItems = (pixelOffsetX: number, pixelOffsetY: number) => {
    ...
    let objInfo: TMap[number][number][number]["o"];
    let grhObj = generateObject();
    ...
    for (let y = minY; y < maxY; y++) {
      ...
      for (let x = minX; x < maxX; x++) {
        ...
        if (objInfo) {
          grhObj = this.inits.graphics[this.inits.objs[objInfo.oi].grhIndex];
  ...
  renderScreen = (tileX, tileY, pixelOffsetX, pixelOffsetY) => {
    ...
    for (let y = minY; y < maxY; y++) {
      ...
      for (let x = minX; x < maxX; x++) {
        ...
        if (objInfo) {
          let grhObj =
            this.inits.graphics[this.inits.objs[objInfo.oi].grhIndex];
      ...
      for (let x = minX; x < maxX; x++) {
        ...
        if (
          ...
          if (this.inits.mapData[this.config.mapNumber][y][x].id) {
            ...
            if (personaje) {
              this.charRender(personaje, tempPixelOffsetX, tempPixelOffsetY);
            }
          }
        }
    ...
    if (this.config.seguroActivado) {
      this.renderText("Seguro Activado", 10, 35, "red", true, 12, "Tahoma");
    } else {
      this.renderText("Seguro Desactivado", 10, 35, "red", true, 12, "Tahoma");
    }
  };
  ...
  drawChar = (personaje, sX, sY) => {
    try {
      ...
      if (this.delta) {
        ...
        if (personaje.idBody) {
          grhRopa = this.inits.bodies[personaje.idBody][personaje.heading];
        }

        if (personaje.idWeapon) {
          grhWeapon = this.inits.armas[personaje.idWeapon][personaje.heading];
        }

        if (personaje.idShield) {
          grhShield = this.inits.escudos[personaje.idShield][personaje.heading];
        }
        ...
        if (
          ...
          if (grhRopa) {
            const CurrentGrhRopa =
              this.inits.graphics[grhRopa].frames[
                Math.ceil(personaje.frameCounter)
              ];
          ...
          if (grhWeapon) {
            const CurrentGrhWeapon =
              this.inits.graphics[grhWeapon].frames[
                Math.ceil(personaje.frameCounterWeapon)
              ];
          ...
          if (grhShield) {
            const CurrentGrhShield =
              this.inits.graphics[grhShield].frames[
                Math.ceil(personaje.frameCounterShield)
              ];
          ...
          if (personaje.idHead) {
            const grhCabeza =
              this.inits.heads[personaje.idHead][personaje.heading];
            ...
            if (this.inits.preCacheGraphics[graphicGrhHead.numFile]) {
              this.canvas.foreground.ctx.drawImage(
                ...
                sY + this.inits.bodies[personaje.idBody].headOffsetY - 18,
                widtHead,
                heightHead
              );
            } else {
          ...
          if (personaje.idHelmet) {
            ...
            const grhHelmet =
              this.inits.cascos[personaje.idHelmet][personaje.heading];
            ...
            if (this.inits.preCacheGraphics[graphicGrhHelmet.numFile]) {
              this.canvas.foreground.ctx.drawImage(
                ...
                sY +
                  this.inits.bodies[personaje.idBody].headOffsetY -
                  18 +
                  casco.offsetY,
                widtHelmet,
                heightHelmet
          ...
          if (this.config.dialogs[personaje.id]) {
            const textLength = this.config.dialogs[personaje.id].msg.length;
            let firstLineLength;
  ...
  drawFx = (personaje, sX, sY, alpha) => {
    try {
      if (this.delta) {
        ...
        personaje.frameFxCounter += this.delta / this.inits.graphics[grh].speed;
        ...
        if (
          ...
          const CurrentGrhIndex =
            this.inits.graphics[grh].frames[
              Math.ceil(personaje.frameFxCounter)
            ];

          const graphicCurrentGrh = this.inits.graphics[CurrentGrhIndex];
          ...
          if (this.inits.preCacheGraphics[graphicCurrentGrh.numFile]) {
            this.canvas.foreground.ctx.drawImage(
    ...
    try {
      ...
      for (let i = 0; i < wordsLength; i++) {
        ...
        if (
          ...
          this.canvas.textos.ctx.strokeText(wordsLine, x - 20, textPosY);
          this.canvas.textos.ctx.fillText(wordsLine, x - 20, textPosY);
          wordsLine = "";
          count = 0;
        }
      }
    ...
    try {
      ...
      if (this.legalPos(tX, tY, this.user.navegando)) {
        ...
        if (this.personajes[this.user.id].heading != direccion) {
          ...
          this.pkg.setPackageID(this.pkg.serverPacketID.changeHeading);
          this.pkg.writeByte(direccion);
          this.config.ws.send(this.pkg.dataSend());
        }
      }
  ...
  hayAgua = (tX, tY) => {
    const g = this.inits.mapa[this.config.mapNumber][tY][tX].g;

    if (
      (g[1] >= 1505 && g[1] <= 1520 && !g[2]) ||
      (g[1] >= 5665 && g[1] <= 5680 && !g[2]) ||
      (g[1] >= 13547 && g[1] <= 13562 && !g[2])
    ) {
      return true;
    } else {
      return false;
  ...
  respawnMe = (idPj, numMap, posX, posY) => {
    try {
      ...
      if (this.config.mapNumber != numMap) {
        ...
        this.inits.mapData[this.config.mapNumber][this.personajes[idPj].pos.y][
          this.personajes[idPj].pos.x
        ].id = 0;
